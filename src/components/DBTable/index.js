...
class DBTable extends React.PureComponent {
  ...
  refresh = async () => {
    ...
    const data = await this.select(this.state.queryObj, this.state.currentPage, this.state.pageSize)
    if (data) {
      this.setState({
        data: data.records,
        total: data.total,
  ...
  async select(queryObj, page, pageSize) {
    ...
    try {
      ...
      const data = await ajax.get(`/${this.tableName}`, queryObj)
      return data
  ...
  handlePageChange = async (page) => {
    ...
    const data = await this.select(this.state.queryObj, page, this.state.pageSize)
    if (data) {
      this.setState({
        currentPage: page,
        data: data.records,
  ...
  handleShowPageChange = async (page, pageSize) => {
    ...
    const data = await this.select(this.state.queryObj, page, pageSize)
    if (data) {
      this.setState({
        currentPage: page,
        data: data.records,
