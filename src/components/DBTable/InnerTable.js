...
class InnerTable extends React.PureComponent {
    ...
    try {
      ...
      if (data) {
        notification.success({
          message: '新增成功',
          description: this.primaryKey ? `新增数据行 主键=${data[this.primaryKey]}` : '',
          duration: 3
        })
  ...
  async handleUpdate(obj, keys = this.state.selectedRowKeys) {
    ...
    try {
      const data = await ajax.put(`/${this.props.tableName}`, { id: keys[0], ...obj })
      hide()
      if (data) {
        notification.success({
          message: '更新成功',
  ...
  async handleDelete(keys = this.state.selectedRowKeys) {
    ...
    try {
      const data = await ajax.delete(`/${this.props.tableName}`, keys)
      hide()
      if (data) {
        notification.success({
          message: '删除成功',
          description: `删除${data}条数据`,
          duration: 3
        })
        ...
        <div className="db-table-button" >
          <Affix offsetTop={8} target={() => document.getElementById('main-content-div')} >
            <Button.Group >
              ...
              <Button type="primary" disabled={!hasSelected || !this.primaryKey || multiSelected}
                      onClick={this.onClickUpdate} >
                {/*<Icon type="edit" /> {multiSelected ? '批量修改' : '修改'} 禁用批量修改*/}
                <Icon type="edit" />修改
              </Button >}
