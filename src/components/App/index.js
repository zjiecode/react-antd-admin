...
import http from '../../sm-utils/Http'
import localStorage from '../../sm-utils/LocalStorage'
...
class App extends React.Component {
  ...
  async componentDidMount() {
    if (!this.props.login) {
      ...
      try {
        // 先去服务端验证下, 说不定已经登录了
        const u = localStorage.get('user')
        const token = u ? u.token : null
        const user = await http.get('/user/token', { token })
        // const res = await ajax.getCurrentUser().catch(() => hide())
  ...
  parseTabTitle() {
    ...
    tabTitleMap.set('*', <span className="ant-layout-tab-text" ><Icon
      type="frown-o" />Error</span >)
    return tabTitleMap
  }
  ...
  updateTab(props) {
    ...
    if (config.tabMode.allowDuplicate === true) {
      if (!this.tabCounter) {
        this.tabCounter = 0
      }
  ...
  render() {
    ...
    if (this.state.tryingLogin) {
      return <div className="center-div" ><Spin spinning size="large" /></div >
    }
...
const mapDispatchToProps = (dispatch) => {
  return {
    handleLoginSuccess: bindActionCreators(loginSuccessActionCreator, dispatch)  // loginSuccess事件比较特殊, 不只Login组件会触发, App组件也会触发
  }
}
